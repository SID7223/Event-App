...
async function first(db, sql, ...params) {
  try {
    const stmt = db.prepare(sql);
    const result = await (params.length > 0 ? stmt.bind(...params) : stmt).first();
    return result ?? null;
  } catch (err) {
...
var createEventSchema = external_exports.object({
  ...
  price: external_exports.number().min(0).optional().default(0),
  location: external_exports.string().optional().default(""),
  date: external_exports.string().min(1, "Date is required"),
  time: external_exports.string().optional().default(""),
  organizerId: external_exports.string().optional(),
  venueId: external_exports.string().optional(),
  neighborhood: external_exports.string().optional(),
  city: external_exports.string().optional().default("lahore"),
  bookingType: external_exports.enum(["external_link", "whatsapp", "in_app"]).optional().default("in_app"),
  externalLink: external_exports.string().url().optional().nullable(),
  whatsappNumber: external_exports.string().optional().nullable(),
  dataSource: external_exports.enum(["ticketwala", "bookme", "manual_entry", "user_host"]).optional().default("manual_entry"),
  image: external_exports.string().optional(),
  featured: external_exports.boolean().optional().default(false),
  featuredUntil: external_exports.string().optional()
});
var updateEventSchema = createEventSchema.partial();
var eventQuerySchema = external_exports.object({
  category: external_exports.string().optional(),
  ...
  sort: external_exports.enum(["date", "popular", "rating"]).optional().default("date")
});

// src/routes/events.ts
function mapEvent(row, categoryName, isFavorited = false) {
  return {
    id: row.id,
    title: row.title,
    ...
    imageId: row.image_id || void 0,
    price: row.price,
    location: row.location,
    date: row.date,
    time: row.time,
    organizer: row.org_name || row.organizer_id || "",
    organizerId: row.organizer_id || void 0,
    venue: row.venue_name || row.venue_id || "",
    venueId: row.venue_id || void 0,
    attendees: row.attendees,
    rating: row.rating,
    isFavorite: isFavorited,
    isFeatured: row.is_featured === 1,
    featuredUntil: row.featured_until || void 0,
    neighborhood: row.neighborhood || void 0,
    city: row.city,
...
function buildWhereClause(query3) {
  const conditions = ["e.status = 'active'", "e.date >= date('now')"];
  const params = [];
  if (query3.category) {
    conditions.push("(c.name = ? OR c.id = ?)");
    params.push(query3.category, query3.category);
  }
  if (query3.city) {
    conditions.push("e.city = ?");
    params.push(query3.city);
  }
  if (query3.neighborhood) {
    conditions.push("e.neighborhood = ?");
    params.push(query3.neighborhood);
  }
  if (query3.q) {
...
function register(app2) {
  app2.get("/events", optionalAuth, async (c) => {
    const queryParams = eventQuerySchema.parse(c.req.query());
    const { sql: where, params } = buildWhereClause(c.req.query());
  ...
  app2.get("/events/featured", async (c) => {
    const events = await query(
      c.env.chillingz_db,
  ...
  app2.get("/events/popular", async (c) => {
    const events = await query(
      c.env.chillingz_db,
  ...
  app2.get("/events/upcoming", async (c) => {
    const events = await query(
      c.env.chillingz_db,
  ...
  app2.get("/events/:id", optionalAuth, async (c) => {
  ...
  app2.post("/events", requireAuth, zValidator("json", createEventSchema), async (c) => {
    ...
    return success(c, mapEvent(created3), 201);
  });
  app2.put("/events/:id", requireAuth, zValidator("json", updateEventSchema), async (c) => {
    const id = c.req.param("id");
    ...
    if (body.neighborhood !== void 0) {
      updates.push("neighborhood = ?");
      params.push(body.neighborhood);
    }
    if (body.city !== void 0) {
      updates.push("city = ?");
      params.push(body.city);
    }
    if (body.bookingType !== void 0) {
      updates.push("booking_type = ?");
      params.push(body.bookingType);
    }
    if (body.externalLink !== void 0) {
      updates.push("external_link = ?");
      params.push(body.externalLink);
    }
    ...
    if (body.featuredUntil !== void 0) {
      updates.push("featured_until = ?");
      params.push(body.featuredUntil || null);
    }
  ...
  app2.delete("/events/:id", requireAuth, async (c) => {
    const id = c.req.param("id");
    const existing = await first(
      c.env.chillingz_db,
...
function mapVenue(row) {
  return {
    ...
    logoId: row.logo_id || void 0,
    coverId: row.cover_id || void 0,
    bio: row.bio,
    address: row.address,
...
__name(mapEventSummary, "mapEventSummary");
function register2(app2) {
  app2.get("/venues", async (c) => {
    ...
    const venues = await query(c.env.chillingz_db, sql, ...params);
    return success(c, venues.map(mapVenue));
  });
  app2.get("/venues/:id", async (c) => {
    const id = c.req.param("id");
    const venue = await first(
      c.env.chillingz_db,
  ...
  app2.get("/venues/:id/events", async (c) => {
    const id = c.req.param("id");
    const venue = await first(
      c.env.chillingz_db,
  ...
  app2.get("/organizers/:id", async (c) => {
    const id = c.req.param("id");
    const organizer = await first(
      c.env.chillingz_db,
    ...
    return success(c, {
      ...
      avatarId: organizer.logo_id || void 0,
      coverId: organizer.cover_id || void 0,
      bio: organizer.bio,
  ...
  app2.get("/organizers/:id/events", async (c) => {
    const id = c.req.param("id");
    const organizer = await first(
      c.env.chillingz_db,
...
var ticketInputSchema = external_exports.object({
...
var createBookingSchema = external_exports.object({
  ...
  tickets: external_exports.array(ticketInputSchema).min(1, "At least one ticket required"),
  paymentMethod: external_exports.string().optional().default("credit_card"),
  totalAmount: external_exports.number().min(0)
});

// src/routes/bookings.ts
function nanoid2(size = 12) {
  const chars = "abcdefghijklmnopqrstuvwxyz0123456789";
  let id = "";
  const bytes = crypto.getRandomValues(new Uint8Array(size));
  for (let i = 0; i < size; i++) {
...
function register3(app2) {
  app2.get("/bookings", requireAuth, async (c) => {
    const userId = c.get("userId");
  ...
  app2.get("/bookings/:id", requireAuth, async (c) => {
    const id = c.req.param("id");
    const userId = c.get("userId");
    const booking = await first(
      c.env.chillingz_db,
  ...
  app2.post("/bookings", requireAuth, zValidator("json", createBookingSchema), async (c) => {
    const body = c.req.valid("json");
    const userId = c.get("userId");
    const event = await first(
      c.env.chillingz_db,
    ...
    const bookingId = `bkg_${nanoid2(12)}`;
    ...
    for (const ticket of body.tickets) {
      const ticketId = `tkt_${nanoid2(12)}`;
      await run(
        c.env.chillingz_db,
  ...
  app2.get("/tickets", requireAuth, async (c) => {
    const userId = c.get("userId");
    const tickets = await query(
      c.env.chillingz_db,
...
__name(mapShowtime, "mapShowtime");
...
__name(mapCinema, "mapCinema");
function register4(app2) {
  app2.get("/movies", async (c) => {
    const city = c.req.query("city");
    ...
    const movies = await query(c.env.chillingz_db, sql, ...params);
  ...
  app2.get("/movies/:id", async (c) => {
    const id = c.req.param("id");
    const movie = await first(
      c.env.chillingz_db,
  ...
  app2.get("/movies/:id/showtimes", async (c) => {
    const id = c.req.param("id");
    const movie = await first(
      c.env.chillingz_db,
  ...
  app2.get("/cinemas", async (c) => {
    ...
    sql += " ORDER BY name";
    const cinemas = await query(c.env.chillingz_db, sql, ...params);
    return success(c, cinemas.map(mapCinema));
  });
  app2.get("/cinemas/:id/showtimes", async (c) => {
    const id = c.req.param("id");
    const cinema = await first(
      c.env.chillingz_db,
...
__name(mapRestaurant, "mapRestaurant");
function register5(app2) {
  app2.get("/restaurants", async (c) => {
    ...
    if (has_live_music === "true") {
      conditions.push("has_live_music = 1");
    }
    if (q) {
    ...
    const restaurants = await query(c.env.chillingz_db, sql, ...params);
    return success(c, restaurants.map(mapRestaurant));
  });
  app2.get("/restaurants/:id", async (c) => {
    const id = c.req.param("id");
...
var updateProfileSchema = external_exports.object({
  ...
  avatarId: external_exports.string().optional(),
  bio: external_exports.string().max(500).optional(),
  username: external_exports.string().min(3).max(50).regex(/^[a-z0-9._]+$/, "Username can only contain lowercase letters, numbers, dots and underscores").optional(),
  gender: external_exports.enum(["male", "female", "other"]).optional()
});
var updatePreferencesSchema = external_exports.object({
  preferences: external_exports.array(external_exports.string()).min(1, "At least one preference required")
});
var updateLocationSchema = external_exports.object({
  city: external_exports.string().min(1, "City is required"),
  neighborhood: external_exports.string().min(1, "Neighborhood is required"),
  ...
  fullAddress: external_exports.string().optional()
});

// src/routes/users.ts
function nanoid3(size = 12) {
  const chars = "abcdefghijklmnopqrstuvwxyz0123456789";
  let id = "";
  const bytes = crypto.getRandomValues(new Uint8Array(size));
  for (let i = 0; i < size; i++) {
...
function register6(app2) {
  app2.get("/users/me", requireAuth, async (c) => {
    const userId = c.get("userId");
    const user = await first(
      c.env.chillingz_db,
    ...
    return success(c, {
      ...
      phone: user.phone || "",
      avatar: user.avatar_url || "",
      avatarId: user.avatar_id || void 0,
      ...
      location: {
        ...
        latitude: user.latitude || void 0,
        longitude: user.longitude || void 0
      },
      createdAt: user.created_at
    });
  });
  app2.put("/users/me", requireAuth, zValidator("json", updateProfileSchema), async (c) => {
    const body = c.req.valid("json");
    const userId = c.get("userId");
    const updates = [];
    const params = [];
  ...
  app2.put("/users/me/preferences", requireAuth, zValidator("json", updatePreferencesSchema), async (c) => {
    const body = c.req.valid("json");
    const userId = c.get("userId");
    await run(
      c.env.chillingz_db,
    ...
    await run(
      ...
      nanoid3(),
      userId,
      JSON.stringify({ preferences: body.preferences })
    );
    return success(c, { preferences: body.preferences });
  });
  app2.put("/users/me/location", requireAuth, zValidator("json", updateLocationSchema), async (c) => {
    const body = c.req.valid("json");
    const userId = c.get("userId");
    await run(
      c.env.chillingz_db,
    ...
    await run(
      ...
      nanoid3(),
      userId,
      JSON.stringify({ city: body.city, neighborhood: body.neighborhood })
    );
    return success(c, {
      city: body.city,
...
function register7(app2) {
  app2.post("/favorites/:eventId", requireAuth, async (c) => {
  ...
  app2.delete("/favorites/:eventId", requireAuth, async (c) => {
    const eventId = c.req.param("eventId");
    const userId = c.get("userId");
    const existing = await first(
      c.env.chillingz_db,
  ...
  app2.get("/favorites", requireAuth, async (c) => {
    const userId = c.get("userId");
    const eventIds = await query(
      c.env.chillingz_db,
  ...
  app2.get("/follows", requireAuth, async (c) => {
    const userId = c.get("userId");
    ...
    const venues = venueIds.length > 0 ? await query(
      c.env.chillingz_db,
      `SELECT id, name FROM venues WHERE id IN (${venueIds.map(() => "?").join(",")})`,
      ...venueIds
    ) : [];
    const organizers = organizerIds.length > 0 ? await query(
      c.env.chillingz_db,
  ...
  app2.post("/follows/venue/:id", requireAuth, async (c) => {
  ...
  app2.delete("/follows/venue/:id", requireAuth, async (c) => {
    const entityId = c.req.param("id");
    const userId = c.get("userId");
    const existing = await first(
      c.env.chillingz_db,
  ...
  app2.post("/follows/organizer/:id", requireAuth, async (c) => {
    const entityId = c.req.param("id");
    const userId = c.get("userId");
    const organizer = await first(
      c.env.chillingz_db,
  ...
  app2.delete("/follows/organizer/:id", requireAuth, async (c) => {
    const entityId = c.req.param("id");
    const userId = c.get("userId");
    const existing = await first(
      c.env.chillingz_db,
  ...
  app2.get("/friends", requireAuth, async (c) => {
    const userId = c.get("userId");
    const filtered = c.req.query("filter");
    let condition = "AND f.status = 'accepted'";
    let order = "f.created_at DESC";
    if (filtered === "pending") {
      condition = "AND f.status = 'pending' AND f.requester_id = ?";
      order = "f.created_at DESC";
    } else if (filtered === "requests") {
      condition = "AND f.status = 'pending' AND f.friend_id = ?";
      order = "f.created_at DESC";
    ...
    if (filtered === "requests") {
      const requestUsers = await query(
        c.env.chillingz_db,
  ...
  app2.get("/friends/suggestions", requireAuth, async (c) => {
    const userId = c.get("userId");
    ...
    const existingFriendIds = await query(
      c.env.chillingz_db,
  ...
  app2.get("/friends/search", requireAuth, async (c) => {
  ...
  app2.post("/friends/request/:id", requireAuth, async (c) => {
    const friendId = c.req.param("id");
    const userId = c.get("userId");
    if (userId === friendId) throw new ValidationError("Cannot add yourself as a friend");
    const user = await first(c.env.chillingz_db, "SELECT id FROM users WHERE id = ?", friendId);
  ...
  app2.post("/friends/accept/:id", requireAuth, async (c) => {
  ...
  app2.post("/friends/reject/:id", requireAuth, async (c) => {
    const requesterId = c.req.param("id");
    const userId = c.get("userId");
    const request = await first(
      c.env.chillingz_db,
  ...
  app2.delete("/friends/:id", requireAuth, async (c) => {
    const friendId = c.req.param("id");
    const userId = c.get("userId");
    const existing = await first(
      c.env.chillingz_db,
  ...
  app2.get("/friends/pending-count", requireAuth, async (c) => {
    const userId = c.get("userId");
  ...
  const privacySchema = external_exports.object({
    hideRSVPs: external_exports.boolean()
  });
  app2.get("/privacy", requireAuth, async (c) => {
    const userId = c.get("userId");
  ...
  app2.put("/privacy", requireAuth, zValidator("json", privacySchema), async (c) => {
    const body = c.req.valid("json");
    const userId = c.get("userId");
    await run(
      c.env.chillingz_db,
...
__name(nanoid5, "nanoid");
var submitEventSchema = external_exports.object({
  title: external_exports.string().min(3, "Title must be at least 3 characters"),
  category: external_exports.string().optional(),
  description: external_exports.string().min(10, "Description must be at least 10 characters"),
  date: external_exports.string().min(1, "Date is required"),
  time: external_exports.string().optional().default("19:00"),
  venueName: external_exports.string().min(1, "Venue name is required"),
  neighborhood: external_exports.string().min(1, "Neighborhood is required"),
  address: external_exports.string().min(1, "Address is required"),
  price: external_exports.number().min(0).optional().default(0),
  imageUrl: external_exports.string().optional(),
  externalLink: external_exports.string().optional()
});
function register8(app2) {
  app2.post("/events/hosted", requireAuth, zValidator("json", submitEventSchema), async (c) => {
    const body = c.req.valid("json");
    const userId = c.get("userId");
    const pendingId = `pevt_${nanoid5(12)}`;
    let categoryId = null;
    if (body.category) {
      const cat = await first(
        c.env.chillingz_db,
  ...
  app2.get("/events/hosted", requireAuth, async (c) => {
    const userId = c.get("userId");
    const events = await query(
      c.env.chillingz_db,
...
__name(mapNotification, "mapNotification");
function register9(app2) {
  app2.get("/notifications", requireAuth, async (c) => {
    const userId = c.get("userId");
  ...
  app2.put("/notifications/:id/read", requireAuth, async (c) => {
    const id = c.req.param("id");
    const userId = c.get("userId");
    const existing = await first(
      c.env.chillingz_db,
  ...
  app2.put("/notifications/read-all", requireAuth, async (c) => {
    const userId = c.get("userId");
    await run(
      c.env.chillingz_db,
...
__name(register9, "register");

// src/routes/categories.ts
function register10(app2) {
  ...
  app2.get("/vibes", async (c) => {
    ...
    const data = await Promise.all(vibes.map(async (vibe) => {
      ...
      return {
        ...
        label: vibe.label,
        icon: vibe.icon,
        categories: categories.filter((c2) => c2 !== null).map((c2) => c2.name)
      };
    }));
    return success(c, data);
  });
}
__name(register10, "register");

// src/routes/search.ts
function register11(app2) {
  app2.get("/search", async (c) => {
    ...
    return success(c, {
      ...results,
      categories: categories.map((c2) => ({ id: c2.id, name: c2.name, icon: c2.icon, color: c2.color, count: 0 })),
      total: results.events?.length ?? 0
    });
  });
}
__name(register11, "register");

// src/routes/dms.ts
function nanoid6(size = 12) {
  const chars = "abcdefghijklmnopqrstuvwxyz0123456789";
  let id = "";
  const bytes = crypto.getRandomValues(new Uint8Array(size));
...
function register12(app2) {
  app2.get("/conversations", requireAuth, async (c) => {
    const userId = c.get("userId");
  ...
  const createConversationSchema = external_exports.object({
    participantIds: external_exports.array(external_exports.string()).min(1).max(10),
    title: external_exports.string().optional()
  });
  app2.post("/conversations", requireAuth, zValidator("json", createConversationSchema), async (c) => {
    const userId = c.get("userId");
    const { participantIds, title } = c.req.valid("json");
    const allParticipantIds = [.../* @__PURE__ */ new Set([userId, ...participantIds])];
    ...
    const convId = nanoid6();
    await run(
      c.env.chillingz_db,
      "INSERT INTO conversations (id, type, title, created_by) VALUES (?, ?, ?, ?)",
      convId,
      allParticipantIds.length === 2 ? "direct" : "group",
      title || null,
      userId
    );
    for (const pid of allParticipantIds) {
      await run(
        ...
        convId,
        pid,
        pid === userId ? "admin" : "member"
      );
    }
    return created(c, { id: convId, existing: false });
  });
  app2.get("/conversations/:id", requireAuth, async (c) => {
    const conversationId = c.req.param("id");
    const userId = c.get("userId");
    const conv = await first(c.env.chillingz_db, "SELECT * FROM conversations WHERE id = ?", conversationId);
  ...
  app2.get("/conversations/:id/messages", requireAuth, async (c) => {
    const conversationId = c.req.param("id");
    const userId = c.get("userId");
    ...
    if (!participant) throw new NotFoundError("Conversation", conversationId);
    const messages = cursor ? await query(
      c.env.chillingz_db,
  ...
  const sendMessageSchema = external_exports.object({
    content: external_exports.string().min(1).max(5e3),
    type: external_exports.enum(["text", "image", "file"]).default("text"),
    mediaUrl: external_exports.string().optional(),
    replyToId: external_exports.string().optional()
  });
  app2.post("/conversations/:id/messages", requireAuth, zValidator("json", sendMessageSchema), async (c) => {
    const conversationId = c.req.param("id");
    const userId = c.get("userId");
  ...
  const editMessageSchema = external_exports.object({ content: external_exports.string().min(1).max(5e3) });
  app2.patch("/messages/:id", requireAuth, zValidator("json", editMessageSchema), async (c) => {
    ...
    const elapsed = (Date.now() - new Date(msg.created_at).getTime()) / 1e3;
    ...
    return success(c, { id: messageId, content, updatedAt: (/* @__PURE__ */ new Date()).toISOString() });
  });
  app2.delete("/messages/:id", requireAuth, async (c) => {
    const messageId = c.req.param("id");
    const userId = c.get("userId");
    const msg = await first(
      c.env.chillingz_db,
  ...
  app2.post("/conversations/:id/read", requireAuth, async (c) => {
    const conversationId = c.req.param("id");
    const userId = c.get("userId");
    await run(
      c.env.chillingz_db,
  ...
  app2.post("/conversations/:id/typing", requireAuth, async (c) => {
    const conversationId = c.req.param("id");
    const userId = c.get("userId");
  ...
  app2.put("/conversations/:id/mute", requireAuth, async (c) => {
    const conversationId = c.req.param("id");
    const userId = c.get("userId");
...
__name(register12, "register");

// src/lib/pusher.ts
function getPusherConfig(env) {
...
__name(getPusherConfig, "getPusherConfig");
function authorizeChannel(config, socketId, channelName) {
  const authData = `${socketId}:${channelName}`;
  const sig = hmacSha256(config.secret, authData);
  return `${config.key}:${sig}`;
}
__name(authorizeChannel, "authorizeChannel");
...
function register13(app2) {
  app2.post("/pusher/auth", requireAuth, async (c) => {
    const userId = c.get("userId");
    ...
    const config = getPusherConfig(c.env);
    if (!config) {
...
__name(register13, "register");

// src/routes/tickets.ts
function nanoid7(size = 12) {
  const chars = "abcdefghijklmnopqrstuvwxyz0123456789";
  let id = "";
  const bytes = crypto.getRandomValues(new Uint8Array(size));
  for (let i = 0; i < size; i++) id += chars[bytes[i] % chars.length];
...
__name(nanoid7, "nanoid");
function register14(app2) {
  app2.get("/tickets", requireAuth, async (c) => {
    const userId = c.get("userId");
    ...
    return paginated(c, tickets.map((t) => ({
      id: t.id,
      eventId: t.event_id,
      ...
      status: t.status,
      rsvpAt: t.created_at,
      isUpcoming: t.start_time > (/* @__PURE__ */ new Date()).toISOString()
    })), page, limit, total?.count || 0);
  });
  const rsvpSchema = external_exports.object({
    status: external_exports.enum(["going", "interested", "maybe"]).default("going"),
    plusOne: external_exports.boolean().default(false)
  });
  app2.post("/events/:id/rsvp", requireAuth, zValidator("json", rsvpSchema), async (c) => {
    const eventId = c.req.param("id");
    const userId = c.get("userId");
    const { status, plusOne } = c.req.valid("json");
    const event = await first(c.env.chillingz_db, "SELECT * FROM events WHERE id = ?", eventId);
    if (!event) throw new NotFoundError("Event", eventId);
    if (event.start_time < (/* @__PURE__ */ new Date()).toISOString()) throw new ValidationError("Cannot RSVP to past events");
    const existing = await first(
      c.env.chillingz_db,
  ...
  app2.delete("/events/:id/rsvp", requireAuth, async (c) => {
    const eventId = c.req.param("id");
    const userId = c.get("userId");
    await run(
      c.env.chillingz_db,
  ...
  app2.get("/events/:id/friends-attending", requireAuth, async (c) => {
    const eventId = c.req.param("id");
    const userId = c.get("userId");
    const friends = await query(
      c.env.chillingz_db,
    ...
    return success(c, friends.map((f) => ({
      userId: f.id,
  ...
  app2.get("/events/:id/attendees", async (c) => {
    const eventId = c.req.param("id");
    ...
    return success(c, attendees.map((a) => ({
      userId: a.id,
...
__name(register14, "register");

// src/routes/creator.ts
function nanoid8(size = 12) {
  const chars = "abcdefghijklmnopqrstuvwxyz0123456789";
  let id = "";
  const bytes = crypto.getRandomValues(new Uint8Array(size));
  for (let i = 0; i < size; i++) id += chars[bytes[i] % chars.length];
...
function register15(app2) {
  const requireCreator = /* @__PURE__ */ __name(async (c, next) => {
    await requireAuth(c, async () => {
      const user = await first(c.env.chillingz_db, "SELECT role FROM users WHERE id = ?", c.get("userId"));
      if (!user || user.role === "user") throw new ForbiddenError("Creator account required");
      await next();
    });
  }, "requireCreator");
  const applySchema = external_exports.object({
    businessName: external_exports.string().min(1).max(200),
    businessType: external_exports.string().min(1).max(100),
    phone: external_exports.string().min(7).max(20),
    documentUrl: external_exports.string().optional()
  });
  app2.post("/creator/apply", requireAuth, zValidator("json", applySchema), async (c) => {
    const userId = c.get("userId");
  ...
  app2.get("/creator/dashboard", requireCreator, async (c) => {
    const userId = c.get("userId");
    const totalEvents = await first(
      c.env.chillingz_db,
      "SELECT COUNT(*) as count FROM events WHERE created_by = ?",
      userId
    );
    const upcomingEvents = await first(
      c.env.chillingz_db,
      "SELECT COUNT(*) as count FROM events WHERE created_by = ? AND start_time > datetime('now')",
      userId
    );
    const totalRsvps = await first(
      c.env.chillingz_db,
      "SELECT COUNT(*) as count FROM event_rsvps er JOIN events e ON e.id = er.event_id WHERE e.created_by = ?",
      userId
    );
    ...
    const topEvent = recentEvents.length > 0 ? recentEvents.reduce((max, e) => e.rsvp_count > max.rsvp_count ? e : max, recentEvents[0]) : null;
    return success(c, {
      stats: {
        totalEvents: totalEvents?.count || 0,
        upcomingEvents: upcomingEvents?.count || 0,
        totalRsvps: totalRsvps?.count || 0,
        totalViews: recentEvents.reduce((s, e) => s + (e.view_count || 0), 0)
      },
  ...
  app2.get("/creator/events", requireCreator, async (c) => {
    const userId = c.get("userId");
    const page = parseInt(c.req.query("page") || "1");
    const limit = Math.min(parseInt(c.req.query("limit") || "20"), 50);
    const offset = (page - 1) * limit;
    ...
    return paginated(c, events.map((e) => ({
      id: e.id,
      title: e.title,
      description: e.description,
      type: e.type,
      ...
      viewCount: e.view_count || 0,
      image: e.image_url,
      status: e.start_time > (/* @__PURE__ */ new Date()).toISOString() ? "upcoming" : "past",
      createdAt: e.created_at
    })), page, limit, total?.count || 0);
  });
  const createEventSchema2 = external_exports.object({
    title: external_exports.string().min(1).max(200),
    description: external_exports.string().min(1).max(5e3),
    type: external_exports.enum(["concert", "festival", "club", "comedy", "art", "sports", "film", "other"]),
    startTime: external_exports.string(),
    endTime: external_exports.string().optional(),
    location: external_exports.string().min(1).max(500),
    latitude: external_exports.number().optional(),
    longitude: external_exports.number().optional(),
    imageUrl: external_exports.string().optional(),
  ...
  app2.post("/creator/events", requireCreator, zValidator("json", createEventSchema2), async (c) => {
    const userId = c.get("userId");
    const data = c.req.valid("json");
    const id = nanoid8();
    const now = (/* @__PURE__ */ new Date()).toISOString();
    const agentName = c.env.AI?.run ? "event-app" : void 0;
  ...
  const updateEventSchema2 = external_exports.object({
  ...
  app2.patch("/creator/events/:id", requireCreator, zValidator("json", updateEventSchema2), async (c) => {
    const eventId = c.req.param("id");
    const userId = c.get("userId");
    const data = c.req.valid("json");
    const event = await first(
  ...
  app2.delete("/creator/events/:id", requireCreator, async (c) => {
    const eventId = c.req.param("id");
    const userId = c.get("userId");
    const event = await first(
      c.env.chillingz_db,
  ...
  app2.get("/creator/rsvps/:eventId", requireCreator, async (c) => {
    const eventId = c.req.param("id");
    const userId = c.get("userId");
    const event = await first(
      c.env.chillingz_db,
...
__name(register15, "register");

// src/routes/admin.ts
function register16(app2) {
  ...
  app2.get("/admin/professional-applications", requireAdmin, async (c) => {
    const page = parseInt(c.req.query("page") || "1");
    const limit = Math.min(parseInt(c.req.query("limit") || "20"), 50);
    const offset = (page - 1) * limit;
  ...
  const reviewSchema = external_exports.object({
    status: external_exports.enum(["approved", "rejected"]),
    notes: external_exports.string().optional()
  });
  app2.patch("/admin/professional-applications/:id", requireAdmin, zValidator("json", reviewSchema), async (c) => {
    const appId = c.req.param("id");
    const adminId = c.get("userId");
    const { status } = c.req.valid("json");
    const app3 = await first(
      c.env.chillingz_db,
      "SELECT * FROM pending_professional_accounts WHERE id = ?",
      appId
    );
    if (!app3) throw new NotFoundError("Application", appId);
    await run(
      c.env.chillingz_db,
  ...
  app2.get("/admin/showtimes", requireAdmin, async (c) => {
    const page = parseInt(c.req.query("page") || "1");
    const limit = Math.min(parseInt(c.req.query("limit") || "20"), 50);
    const offset = (page - 1) * limit;
    const showtimes = await query(
    ...
    return paginated(c, showtimes.map((s) => ({
      id: s.id,
      title: s.title,
      type: s.type,
      ...
      status: s.start_time > (/* @__PURE__ */ new Date()).toISOString() ? "upcoming" : "past"
    })), page, limit, total?.count || 0);
  });
  app2.patch("/admin/showtimes/:id/feature", requireAdmin, async (c) => {
    const eventId = c.req.param("id");
  ...
  app2.get("/admin/users", requireAdmin, async (c) => {
    const page = parseInt(c.req.query("page") || "1");
    const limit = Math.min(parseInt(c.req.query("limit") || "20"), 50);
    const offset = (page - 1) * limit;
    ...
    return paginated(c, users.map((u) => ({
      id: u.id,
      name: `${u.first_name} ${u.last_name}`,
      email: u.email,
      avatar: u.avatar_url || "",
  ...
  app2.patch("/admin/users/:id/role", requireAdmin, zValidator("json", external_exports.object({ role: external_exports.enum(["user", "creator", "admin"]) })), async (c) => {
    const targetId = c.req.param("id");
    const { role } = c.req.valid("json");
    const user = await first(c.env.chillingz_db, "SELECT id FROM users WHERE id = ?", targetId);
...
function fromHex(hex2) {
  ...
  for (let i = 0; i < hex2.length; i += 2) bytes[i / 2] = parseInt(hex2.substring(i, i + 2), 16);
  return bytes;
}
  ...
  return async (c, next) => {
    ...
    if (db) {
      ...
      if (row) {
        ...
        ).bind(cacheKey, String(Math.ceil(opts.windowMs / 1e3))).run();
      }
    }
    await next();
  };
...
function authRateLimit() {
  return rateLimit({ windowMs: 9e5, max: 5, message: "Too many login attempts. Try again in 15 minutes." });
}
__name(authRateLimit, "authRateLimit");

// src/templates/reset-password.ts
function buildResetPasswordHtml(props) {
...
__name(buildResetPasswordHtml, "buildResetPasswordHtml");

// src/routes/auth.ts
function nanoid9(size = 12) {
  const chars = "abcdefghijklmnopqrstuvwxyz0123456789";
  let id = "";
  const bytes = crypto.getRandomValues(new Uint8Array(size));
  for (let i = 0; i < size; i++) id += chars[bytes[i] % chars.length];
...
__name(nanoid9, "nanoid");
var SESSION_TTL_MS = 30 * 24 * 60 * 60 * 1e3;
function register17(app2) {
  ...
  app2.post("/auth/signup", authRateLimit(), zValidator("json", signupSchema), async (c) => {
    const db = c.env.chillingz_db;
    const { email, password, firstName, lastName, phone, gender } = c.req.valid("json");
    const existing = await first(db, "SELECT id FROM users WHERE email = ?", email.toLowerCase());
    if (existing) throw new ConflictError("An account with this email already exists");
    const passwordHash = await hashPassword(password);
    const verificationToken = generateSecureToken();
    const userId = nanoid9();
    await run(
      db,
  ...
  app2.post("/auth/login", authRateLimit(), zValidator("json", loginSchema), async (c) => {
    const db = c.env.chillingz_db;
  ...
  const forgotSchema = external_exports.object({ email: external_exports.string().email() });
  app2.post("/auth/forgot-password", authRateLimit(), zValidator("json", forgotSchema), async (c) => {
    const db = c.env.chillingz_db;
    const { email } = c.req.valid("json");
    const user = await first(db, "SELECT id, first_name, email FROM users WHERE email = ?", email.toLowerCase());
    ...
    try {
      const msg = new EmailMessage("noreply@corlify.com", user.email, { subject, html });
  ...
  const resetSchema = external_exports.object({
    token: external_exports.string().min(1),
    password: external_exports.string().min(8).max(128)
  });
  app2.post("/auth/reset-password", authRateLimit(), zValidator("json", resetSchema), async (c) => {
    const db = c.env.chillingz_db;
  ...
  app2.post("/auth/verify-email", zValidator("json", external_exports.object({ token: external_exports.string().min(1) })), async (c) => {
    const db = c.env.chillingz_db;
    const user = await first(
      db,
  ...
  app2.post("/auth/resend-verification", authRateLimit(), zValidator("json", external_exports.object({ email: external_exports.string().email() })), async (c) => {
    const db = c.env.chillingz_db;
    const { email } = c.req.valid("json");
    const user = await first(db, "SELECT id, email_verified FROM users WHERE email = ?", email.toLowerCase());
    if (!user) return success(c, { sent: true });
  ...
  app2.post("/auth/change-password", requireAuth, zValidator("json", changePasswordSchema), async (c) => {
    const db = c.env.chillingz_db;
    const userId = c.get("userId");
    const { currentPassword, newPassword } = c.req.valid("json");
    const user = await first(db, "SELECT password_hash FROM users WHERE id = ?", userId);
  ...
  app2.get("/auth/sessions", requireAuth, async (c) => {
    const userId = c.get("userId");
  ...
  app2.delete("/auth/sessions/:id", requireAuth, async (c) => {
    const userId = c.get("userId");
...
// .wrangler/tmp/bundle-DeYgft/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-DeYgft/middleware-loader.entry.ts
