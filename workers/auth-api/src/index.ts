  ...
  try {
    ...
    if (existing) {
      ...
      await env.chillingz_db.prepare(
        "UPDATE users SET first_name = ?, last_name = ?, avatar_url = ?, email = ?, updated_at = datetime('now') WHERE id = ?"
      ).bind(googleUser.given_name, googleUser.family_name, googleUser.picture, googleUser.email, userId).run();
    } else {
      userId = `user_${nanoid(12)}`;
      await env.chillingz_db.prepare(
        `INSERT INTO users (id, google_id, email, first_name, last_name, avatar_url, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))`
      ).bind(userId, googleUser.sub, googleUser.email, googleUser.given_name, googleUser.family_name, googleUser.picture).run();
    }
    ...
    ).bind(nanoid(), userId, token, expiresAt).run();
